...
async function turn(action) {
    ...
    try {
        ...
        if (state.turnCount <= 4) {
            contextForFirstPass = state.originalHistory.slice();
        } else {
            const lastCompress = state.lastCompressTurn;
            const unsqueezed = state.originalHistory.slice(lastCompress);
            if (unsqueezed.length <= 3) contextForFirstPass = [state.compressedSummary, ...unsqueezed].filter(Boolean);
            else contextForFirstPass = [state.compressedSummary, ...unsqueezed.slice(-4)].filter(Boolean);
        }
        ...
        const enhancementPrompt = `Придумай 4 случайных слова.  Затем ассоциативно свободно используй их как источник случайности, чтобы создать разнообразный, небанальный и качественный ответ на задачу. Ты не должен употреблять придуманные слова - они лишь источник большего разнообразия конечных токенов твоего ответа:Ты мастер социально-драматической художественной текстовой игры про детство в 1990-х. Ниже текст — увеличь его примерно в 1.5 раза, насыть аутентичными и интереснымм запоминающимися диалогами и описаниями. Исправь очевидные ляпы, ориентируйся на предыдущую историю как на абсолютный канон. Не пиши предисловий и послесловий. Не используй пост-знания и мета-размышления героя об эпохе. Повествование должно исходить изнутри эпохи, а не над эпохой.
...
- знакомые люди:
${npcList || 'Нет'} но не пытайся упомянуть в финальном ответе их всех
- предметы:
${itemList || 'Нет'} но не пытайся упомянуть в ыинальном ответе их все
${summary ? '\n' + summary : ''}
${statsGuidance ? `\nОсобые указания по параметрам персонажа. Вплетай их органично, если уместно. Повествование строго от лица ребёнка ${state.age} лет, язык повествования должен быть уместен возрасту и уму героя:\n${statsGuidance}` : ''}`;
