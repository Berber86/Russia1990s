...
2. Описывай текст глазами ребёнка, уместным для его возраста языком.
...
async function turn(action) {
    ...
    try {
        ...
        const enhancementPrompt = `Ты мастер социально-драматической художественной текстовой игры про детство в 1990-х. Ниже текст — увеличь его примерно в 1.5 раза, литературно перепиши, насыть аутентичными и интереснымм диалогами и описаниями. Исправь очевидные ляпы, ориентируйся на предыдущую историю как на абсолютный канон. Не пиши предисловий и послесловий. Не используй пост-знания и мета-размышления героя об эпохе. Повествование должно исходить изнутри эпохи, а не над эпохой.
...
${itemList || 'Нет'}
${summary ? '\n' + summary : ''}
${statsGuidance ? `\nОсобые указания по параметрам персонажа. Вплетай их органично, если уместно. Повествование строго от лица ребёнка ${state.age} лет, язык повествования должен быть уместен возрасту героя:\n${statsGuidance}` : ''}`;
